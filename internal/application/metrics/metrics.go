...
func New(repo ports.MetricsRepo, onChanged func(context.Context, domain.Snapshot), auditor ports.AuditPublisher) *Service {
	...
	go func() {
		...
		for {
			...
			case msg := <-s.auditQueue:
				_ = s.auditor.Publish(msg.ctx, msg.evt)
			}
		}
	}()
}
...
func (s *Service) Upsert(ctx context.Context, m domain.Metrics, clientIP string) (domain.Metrics, error) {
	...
	case string(domain.Gauge):
		...
		if err == nil {
			s.notifyAudit(ctx, []string{m.ID}, clientIP)
		}
		return res, err
	case string(domain.Counter):
		if m.Delta == nil {
...
func (s *Service) UpsertBatch(ctx context.Context, items []domain.Metrics, clientIP string) (int, error) {
	...
	s.notifyAudit(ctx, names, clientIP)
	if s.onChanged != nil {
...
func (s *Service) notifyAudit(ctx context.Context, names []string, clientIP string) {
	if s == nil || s.auditor == nil {
		return
	}
...
func (s *Service) enqueueAudit(ctx context.Context, evt domain.AuditEvent) {
	if s.auditQueue == nil {
		if err := s.auditor.Publish(ctx, evt); err != nil {
			log.Printf("metrics: audit publish failed (%v)", err)
		}
		return
	}
	select {
