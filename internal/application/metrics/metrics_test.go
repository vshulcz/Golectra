...
func (f *fakeAuditor) Publish(_ context.Context, evt domain.AuditEvent) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeAuditor) Events() []domain.AuditEvent {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([]domain.AuditEvent, len(f.events))
	copy(out, f.events)
	return out
}

func (f *fakeAuditor) WaitForEvents(n int, timeout time.Duration) []domain.AuditEvent {
...
func TestService_Upsert_EmitsAudit(t *testing.T) {
	...
	_, err := svc.Upsert(ctx, domain.Metrics{ID: "Alloc", MType: string(domain.Gauge), Value: ptrFloat64(1.0)}, "10.0.0.1")
	if err != nil {
...
func TestService_UpsertBatch_EmitsAuditDedup(t *testing.T) {
	...
	ctx := context.Background()
	updated, err := svc.UpsertBatch(ctx, items, "192.0.2.1")
	if err != nil {
