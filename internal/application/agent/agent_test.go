...
func TestService_Run_Error(t *testing.T) {
	...
	pub := &fakePublisher{}
	cfg := Config{PollInterval: 1 * time.Millisecond, ReportInterval: 2 * time.Millisecond}
...
func TestService_Run_EmptySnapshot(t *testing.T) {
	...
	pub := &fakePublisher{}
	cfg := Config{PollInterval: 1 * time.Millisecond, ReportInterval: 5 * time.Millisecond}
	svc := New(cfg, coll, pub)
