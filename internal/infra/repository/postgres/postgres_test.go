...
func TestRepo_UpdateMany_Retry(t *testing.T) {
	orig := retry.DefaultBackoff
	retry.DefaultBackoff = []time.Duration{1 * time.Millisecond, 1 * time.Millisecond}
	defer func() { retry.DefaultBackoff = orig }()
...
func TestRepo_Snapshot_Retry(t *testing.T) {
	orig := retry.DefaultBackoff
	retry.DefaultBackoff = []time.Duration{1 * time.Millisecond}
	defer func() { retry.DefaultBackoff = orig }()
...
func TestRepo_GetGauge_ContextCancel(t *testing.T) {
	orig := retry.DefaultBackoff
	retry.DefaultBackoff = []time.Duration{50 * time.Millisecond, 50 * time.Millisecond}
	defer func() { retry.DefaultBackoff = orig }()
