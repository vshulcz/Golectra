...
func TestClient_JoinPath(t *testing.T) {
	c, err := New("http://x:1/base", nil, "", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	c2, _ := New("http://x:1/base/", nil, "", nil)
...
func TestSendOne_VariousResponses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c, err := New(srv.URL, &http.Client{Timeout: 2 * time.Second}, "", nil)
			if err != nil {
				t.Fatal(err)
			}

			val := 123.45
			err = c.SendOne(context.TODO(), domain.Metrics{ID: allocMetricID, MType: gaugeMetricType, Value: &val})
			...
			if got.metric.ID != allocMetricID || got.metric.MType != gaugeMetricType {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = c.SendBatch(context.TODO(), []domain.Metrics{
				{ID: allocMetricID, MType: gaugeMetricType, Value: &val},
...
func TestSendOne_RetryOnNetworkErrors(t *testing.T) {
	orig := retry.DefaultBackoff
	retry.DefaultBackoff = []time.Duration{1 * time.Millisecond, 1 * time.Millisecond, 1 * time.Millisecond}
	defer func() { retry.DefaultBackoff = orig }()
...
func TestSendOne_RetryExhausted(t *testing.T) {
	...
	hc := &http.Client{Transport: rt, Timeout: 2 * time.Second}
	c, _ := New("http://example", hc, "", nil)

	val := 1.0
	err := c.SendOne(context.Background(), domain.Metrics{ID: allocMetricID, MType: gaugeMetricType, Value: &val})
...
func TestSendOne_ContextCancel(t *testing.T) {
	orig := retry.DefaultBackoff
	retry.DefaultBackoff = []time.Duration{50 * time.Millisecond, 50 * time.Millisecond, 50 * time.Millisecond}
	defer func() { retry.DefaultBackoff = orig }()
	...
	val := 10.0
	err := c.SendOne(ctx, domain.Metrics{ID: allocMetricID, MType: gaugeMetricType, Value: &val})
...
func TestSendOne_EncryptedPayload(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.Header.Get(encrypter.HeaderKey()); got != encrypter.HeaderValue() {
			t.Fatalf("header=%q want %q", got, encrypter.HeaderValue())
		}

		raw := mustReadAll(t, r.Body)
		plain, err := decrypter.Decrypt(raw)
		if err != nil {
		...
		payload := mustReadAll(t, gr)

		var m domain.Metrics
		if err := json.Unmarshal(payload, &m); err != nil {
			t.Fatalf("unmarshal: %v", err)
		}
		if m.ID != allocMetricID || m.MType != gaugeMetricType || m.Value == nil {
	...
	c, err := New(srv.URL, nil, "", encrypter)
	if err != nil {
		t.Fatal(err)
	}
