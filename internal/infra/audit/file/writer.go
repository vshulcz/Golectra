...
// Publish marshals the audit event and atomically appends it to the writer's file.
func (w *Writer) Publish(_ context.Context, evt domain.AuditEvent) (retErr error) {
	if w == nil || w.path == "" {
		return nil
	}
