...
func waitForPollCount(st *state.Stats, want int64, timeout time.Duration) bool {
...
func TestPoller_SetsMetricsAndRandomValue(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			go p.Run(tc.interval)

			if ok := waitForPollCount(st, tc.ticks, 500*time.Millisecond); !ok {
				p.Stop()
